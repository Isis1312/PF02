...
namespace PF01	
{
	...
	public partial class login : Form
	{
		...
		private void ValidarYAcceder()
		{
			...
			{
				MessageBox.Show("acceso permitido", "exito" ,MessageBoxButtons.OK, MessageBoxIcon.Error);
    		...
    		if (File.Exists(filePath))
    		{
        		try
        		{
            		using (FileStream fs = new FileStream(filePath, FileMode.Open))
           			{
                		...
                		usuarios = (List<Class1>)bf.Deserialize(fs);
            		}
       			}
            		...
            		MessageBox.Show("Error al cargar usuarios. El archivo puede estar corrupto. Se inicializará una lista vacía.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
           		 	usuarios = new List<Class1>(); // Se reinicia la lista
        		...
        		catch (IOException ex)
        		{
            		...
            		usuarios = new List<Class1>(); // Se inicializa lista vacía en caso de error
        		}
    	}
    		else
    		{
    			usuarios = new List<Class1>(){new Class1("admin", "1234")}; //  Usuario por defecto
		...
		private void CrearUsuario()
		{
            ...
            {
            usuarios.Add(new Class1(nuevoUsuario, nuevaContraseña));
            GuardarUsuario();
